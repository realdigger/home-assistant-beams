...
- Переключатель ручного и автоматического режимов.
- Общая яркость и управление десятью спектральными каналами.
- Выбор сохранённого спектра из галереи контроллера.
...
```text
Настройки → Устройства и службы → Добавить интеграцию → BEAMS Light
```
...
В автоматическом режиме слайдеры общей яркости и отдельных каналов недоступны. Селектор спектра показывает `Авто: дневной цикл`: контроллер интерполирует точки цикла, поэтому единого имени спектра нет.
...
Значения отображаются в диапазоне `0–100 %`. Общая яркость сохраняет относительные уровни каналов. Изменение значения переключает контроллер в ручной режим.
...
Интеграция автоматически регистрирует две пользовательские карточки; добавлять ресурс фронтенда вручную не нужно.
...
Слайдеры используют цвет канала от контроллера и становятся неактивными в автоматическом режиме.
